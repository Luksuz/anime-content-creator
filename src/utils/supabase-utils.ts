...
export async function getValidApiKey(): Promise<string | null> {
  ...
  try {
    ...
    console.log("✅ Retrieved valid API key from database");
    return data.api_key;
 ...
 * @returns Object with counts of valid and invalid API keys
 */
export async function getApiKeyStatistics(): Promise<{ success: boolean; validCount: number; invalidCount: number; totalCount: number; error?: string }> {
  if (!supabaseAdmin) {
    console.error("Supabase Admin client is not initialized. Cannot get API key statistics.");
    return { success: false, validCount: 0, invalidCount: 0, totalCount: 0, error: "Supabase Admin client not initialized." };
  }

  try {
    // Get count of valid API keys
    const { count: validCount, error: validError } = await supabaseAdmin
      .from('api_keys_rezu')
      .select('*', { count: 'exact', head: true })
      .eq('is_valid', true);

    if (validError) {
      console.error("Error fetching valid API key count:", validError);
      return { success: false, validCount: 0, invalidCount: 0, totalCount: 0, error: validError.message };
    }

    // Get count of invalid API keys
    const { count: invalidCount, error: invalidError } = await supabaseAdmin
      .from('api_keys_rezu')
      .select('*', { count: 'exact', head: true })
      .eq('is_valid', false);

    if (invalidError) {
      console.error("Error fetching invalid API key count:", invalidError);
      return { success: false, validCount: 0, invalidCount: 0, totalCount: 0, error: invalidError.message };
    }

    const totalCount = (validCount || 0) + (invalidCount || 0);

    console.log(`✅ API Key Statistics: ${validCount} valid, ${invalidCount} invalid, ${totalCount} total`);
    ...
    return { success: false, validCount: 0, invalidCount: 0, totalCount: 0, error: err.message || "An unexpected error occurred." };
  }
} 